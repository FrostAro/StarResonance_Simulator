...
用于比较"基准配置"与多个"候选配置"的DPS提升，消除程序随机的干扰：

```bash
./bin/dps_simulator_compare [配对数] [起始种子]
```

原理：基准与每个候选在**相同随机种子**下配对模拟（CRN，公共随机数），逐对相减时技能触发/Buff时机的随机互相抵消，因此1%量级的小提升也能稳定测出（20对时t值可达数千）。

修改`main_compare.cpp`：

- `useBeam`：true=射线，false=冰矛
- `base`：基准站街面板
- `buildCandidates()`：用`make([](SimConfig& c){ c.xxx += y; }, "标签")`按行添加要对比的改动
